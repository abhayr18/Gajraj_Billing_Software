 ...
 * Mirrors src/lib/db.ts but runs in Electron's Node.js runtime.
 * Exports an init function that creates/opens the SQLite database
 * at the given path (typically app.getPath('userData')).
 */
 ...
 * Creates the file if it doesn't exist, runs schema + seeds.
 * @param {string} dbDir — directory to store the database file
 * @returns {import('better-sqlite3').Database}
 */
function initDatabase(dbDir) {
    const dbPath = path.join(dbDir, 'gajraj_store.db');
